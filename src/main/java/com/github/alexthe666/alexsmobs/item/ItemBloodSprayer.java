...
public class ItemBloodSprayer extends Item {
    ...
    public static final Predicate<ItemStack> IS_BLOOD = (stack) -> stack.getItem() == AMItemRegistry.BLOOD_SAC;

    public ItemBloodSprayer() {
    ...
    public void onUsingTick(ItemStack stack, EntityLivingBase livingEntityIn, int count) {
        if (isUsable(stack)) {
            if (count % 2 == 0) {
                ...
                EntityMosquitoSpit blood = new EntityMosquitoSpit(livingEntityIn.world, livingEntityIn, !left);
                Vec3d look = livingEntityIn.getLook(1.0F);
                Random rand = livingEntityIn.world.rand;
                livingEntityIn.world.playSound(null, livingEntityIn.posX, livingEntityIn.posY, livingEntityIn.posZ, SoundEvents.BLOCK_LAVA_POP, SoundCategory.NEUTRAL, 1.0F, 1.2F + (rand.nextFloat() - rand.nextFloat()) * 0.2F);
                blood.shoot(look.x, look.y, look.z, 1F, 10);
