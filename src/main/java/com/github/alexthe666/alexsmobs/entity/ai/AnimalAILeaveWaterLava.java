...
public class AnimalAILeaveWaterLava extends EntityAIBase {
    private final EntityCreature creature;
    private BlockPos targetPos;
    private int executionChance = 30;

    public AnimalAILeaveWaterLava(EntityCreature creature) {
    ...
    public boolean shouldExecute() {
        if (isLiquidAt(this.creature.getPosition()) && (this.creature.getAttackTarget() != null || this.creature.getRNG().nextInt(executionChance) == 0)) {
    ...
    public void updateTask() {
        ...
        if (this.creature.collidedHorizontally && (this.creature.isInWater() || this.creature.isInLava())) {
            float f1 = this.creature.rotationYaw * ((float) Math.PI / 180F);
            this.creature.motionX += (double) (-MathHelper.sin(f1) * 0.2F);
            this.creature.motionY += 0.1D;
            this.creature.motionZ += (double) (MathHelper.cos(f1) * 0.2F);
        }
    }
    ...
    public boolean shouldContinueExecuting() {
        ...
        return !this.creature.getNavigator().noPath() && targetPos != null && !isLiquidAt(targetPos);
    }
