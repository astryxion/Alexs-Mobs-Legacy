...
/**
 * 1.12.2 port of 1.16 {@code CrocodileAIRandomSwimming} / {@code RandomWalkingGoal} swimming variant.
 */
public class CrocodileAIRandomSwimming extends EntityAIBase {

    protected final EntityCrocodile creature;
    ...
    public CrocodileAIRandomSwimming(EntityCrocodile creature, double speed, int chance) {
    ...
    public boolean shouldExecute() {
        if (this.creature.isBeingRidden()
                ...
                || (!this.creature.isInWater() && !this.creature.shouldEnterWater())) {
            return false;
        }
        if (!this.mustUpdate) {
    ...
    @Nullable
    private Vec3d findSurfaceTarget(EntityCrocodile creature, int unused1, int unused2) {
