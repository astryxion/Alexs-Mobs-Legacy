...
/**
 * Lava vision: same quads/lighting as vanilla {@link BlockFluidRenderer}, but lava vertex alpha uses
 * {@link AMConfig#lavaOpacity} (matches 1.16 lava vision behavior intent).
 */
public class LavaVisionFluidRenderer extends BlockFluidRenderer {

    /** same instance passed to {@link BlockFluidRenderer}; cached because {@code blockColors} is private in the superclass */
    private final BlockColors blockColorsRef;

    public LavaVisionFluidRenderer(BlockColors blockColorsIn) {
        super(blockColorsIn);
        this.blockColorsRef = blockColorsIn;
    }
    ...
    public boolean renderFluid(IBlockAccess blockAccess, IBlockState blockStateIn, BlockPos blockPosIn, BufferBuilder worldRendererIn) {
        ...
        int i = this.blockColorsRef.colorMultiplier(blockStateIn, blockAccess, blockPosIn, 0);
        ...
        if (flag1) {
            ...
            float f25 = 1.0F * f1;
            float f26 = 1.0F * f2;
            worldRendererIn.pos(d0 + 0.0D, d1 + (double) f7, d2 + 0.0D).color(f24, f25, f26, alpha).tex((double) f13, (double) f17).lightmap(l2, i3).endVertex();
            worldRendererIn.pos(d0 + 0.0D, d1 + (double) f8, d2 + 1.0D).color(f24, f25, f26, alpha).tex((double) f14, (double) f18).lightmap(l2, i3).endVertex();
            worldRendererIn.pos(d0 + 1.0D, d1 + (double) f9, d2 + 1.0D).color(f24, f25, f26, alpha).tex((double) f15, (double) f19).lightmap(l2, i3).endVertex();
            worldRendererIn.pos(d0 + 1.0D, d1 + (double) f10, d2 + 0.0D).color(f24, f25, f26, alpha).tex((double) f16, (double) f20).lightmap(l2, i3).endVertex();
        ...
        if (flag2) {
            ...
            int i2 = l1 >> 16 & 65535;
            int j2 = l1 & 65535;
            worldRendererIn.pos(d0, d1, d2 + 1.0D).color(0.5F, 0.5F, 0.5F, alpha).tex((double) f35, (double) f38).lightmap(i2, j2).endVertex();
            worldRendererIn.pos(d0, d1, d2).color(0.5F, 0.5F, 0.5F, alpha).tex((double) f35, (double) f37).lightmap(i2, j2).endVertex();
            worldRendererIn.pos(d0 + 1.0D, d1, d2).color(0.5F, 0.5F, 0.5F, alpha).tex((double) f36, (double) f37).lightmap(i2, j2).endVertex();
            worldRendererIn.pos(d0 + 1.0D, d1, d2 + 1.0D).color(0.5F, 0.5F, 0.5F, alpha).tex((double) f36, (double) f38).lightmap(i2, j2).endVertex();
            flag3 = true;
        }
        ...
        for (int i1 = 0; i1 < 4; ++i1) {
            ...
            if (aboolean[i1]) {
                ...
                worldRendererIn.pos(d3, d1 + (double) f39, d4).color(f32, f33, f34, alpha).tex((double) f41, (double) f28).lightmap(k, l).endVertex();
                worldRendererIn.pos(d5, d1 + (double) f40, d6).color(f32, f33, f34, alpha).tex((double) f27, (double) f29).lightmap(k, l).endVertex();
                worldRendererIn.pos(d5, d1 + 0.0D, d6).color(f32, f33, f34, alpha).tex((double) f27, (double) f30).lightmap(k, l).endVertex();
                worldRendererIn.pos(d3, d1 + 0.0D, d4).color(f32, f33, f34, alpha).tex((double) f41, (double) f30).lightmap(k, l).endVertex();
    ...
    /**
     * Copy of vanilla {@link BlockFluidRenderer} private method — needed because it is not visible to subclasses.
     */
