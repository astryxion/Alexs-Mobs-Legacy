...
/**
 * Elytra wing angles match vanilla {@link net.minecraft.client.model.ModelElytra}; texture differs per renderer.
 */
@SideOnly(Side.CLIENT)
public class ModelAMElytra extends ModelBase {
    ...
    public ModelAMElytra withAnimations(EntityLivingBase entity) {
        float partialTick = net.minecraft.client.Minecraft.getMinecraft().getRenderPartialTicks();
        ...
        this.setRotationAngles(limbSwing, limbSwingAmount, ageInTicks, 0.0F, 0.0F, 0.0625F, entity);
        return this;
    }
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        ...
        float f2 = 0.0F;
        float f3 = 0.0F;
        boolean elytraFlying = entityIn instanceof EntityLivingBase && ((EntityLivingBase) entityIn).isElytraFlying();
        if (!elytraFlying && entityIn instanceof EntityPlayerSP && entityIn.world.isRemote) {
            EntityPlayerSP player = (EntityPlayerSP) entityIn;
            if (ItemTarantulaHawkElytra.isWearingUsable(player) && !player.onGround && player.fallDistance > 0.0F
                    && player.movementInput.jump) {
                elytraFlying = true;
            }
        }
        if (entityIn instanceof EntityLivingBase && elytraFlying) {
            float f4 = 1.0F;
            EntityLivingBase living = (EntityLivingBase) entityIn;
            if (living.motionY < 0.0D) {
                Vec3d motion = new Vec3d(living.motionX, living.motionY, living.motionZ).normalize();
                f4 = 1.0F - (float) Math.pow(-motion.y, 1.5D);
            }
