...
public class TabulaModelBlock {
   ...
   @VisibleForTesting
   static final Gson SERIALIZER = (new GsonBuilder()).registerTypeAdapter(TabulaModelBlock.class, new Deserializer()).create();
   ...
   private ItemTransformVec3f getTransform(ItemCameraTransforms.TransformType type) {
      return this.parent != null && this.cameraTransforms.getTransform(type) == ItemTransformVec3f.DEFAULT ? this.parent.getTransform(type) : this.cameraTransforms.getTransform(type);
   }
