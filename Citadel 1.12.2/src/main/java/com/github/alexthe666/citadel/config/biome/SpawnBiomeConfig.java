...
public class SpawnBiomeConfig {
   ...
   private SpawnBiomeConfig(ResourceLocation fileName) {
      if (!fileName.getResourcePath().endsWith(".json")) {
         this.fileName = new ResourceLocation(fileName.getResourceDomain(), fileName.getResourcePath() + ".json");
      } else {
   ...
   public static <T> T getOrCreateConfigFile(File configDir, String configName, T defaults, Type type) {
      ...
      if (!configFile.exists()) {
         try {
            ...
            Citadel.LOGGER.error("Spawn Biome Config: Could not write " + configFile.toString(), e);
         }
      }
      ...
      try {
         ...
         Citadel.LOGGER.error("Spawn Biome Config: Could not load " + configFile.toString(), e);
         return defaults;
      }
   }

   private File getConfigDirFile() {
      return new File(Loader.instance().getConfigDir(), this.fileName.getResourceDomain());
   }

   private SpawnBiomeData getConfigData(SpawnBiomeData defaultConfigData) {
      SpawnBiomeData configData = (SpawnBiomeData)getOrCreateConfigFile(this.getConfigDirFile(), this.fileName.getResourcePath(), defaultConfigData, (new TypeToken<SpawnBiomeData>() {
      }).getType());
      return configData;
   }
}
